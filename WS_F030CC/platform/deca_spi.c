...
//    HAL_GPIO_WritePin(DWM1000_SPI_CS_GPIO_Port, DWM1000_SPI_CS_Pin, GPIO_PIN_RESET); // drive select low
    SPIx_CS_GPIO->BRR = SPIx_CS;

    HAL_StatusTypeDef error=0;
    error=HAL_SPI_Transmit(&hspi1, headerBuffer, headerLength, 1000);
    error=HAL_SPI_Receive(&hspi1, readBuffer, readlength, 1000);
...
//    HAL_GPIO_WritePin(DWM1000_SPI_CS_GPIO_Port, DWM1000_SPI_CS_Pin, GPIO_PIN_SET); // drive select high
    SPIx_CS_GPIO->BSRR = SPIx_CS;
